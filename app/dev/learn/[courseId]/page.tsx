...
export default function LearnerRuntimePage({
  ...
  // Read learner token from sessionStorage (set by author page setup)
  const getToken = () => {
  ...
  useEffect(() => {
        ...
        fetch(`/api/courses/${courseId}/runtime`, {
                ...
                if (json.data) {
                          ...
                          setMessage(
                                      json.error || 'No runtime package found. Publish first from the Author page.',
                                    );
                }
        });
  }, [courseId]);
  ...
  const startLesson = async () => {
        ...
        const res = await fetch('/api/lesson-attempts', {
                ...
                body: JSON.stringify({
                          courseId,
                          runtimeLessonId: currentLesson.id,
                }),
        });
  ...
  const completeLesson = async () => {
        ...
        const percent = Number(
                (((lessonIndex + 1) / runtime.lessons.length) * 100).toFixed(2),
              );
        ...
        if (lessonIndex < runtime.lessons.length - 1) {
                ...
                setAttemptId(null);
                setMessage(
                          `Lesson complete! Progress: ${percent}%. Moving to next lesson.`,
                        );
        } else {
                setMessage(
                          `All lessons complete! Progress: ${percent}%. Data: ${JSON.stringify(progressJson)}`,
                        );
        }
  };
  ...
  if (!getToken()) {
        return (
                <main className="p-8">
                        <h1 className="text-xl font-semibold mb-2">No auth token found</h1>h1>
                        <p>
                                  Go to{' '}
                                  <a className="text-blue-700 underline" href="/dev/author">
                                              /dev/author
                                  </a>a>{' '}
                                  and click "0) Dev Setup" first.
                        </p>p>
                </main>main>
              );
  }
    ...
    if (!runtime)
          return (
                  <main className="p-8">
                          <p>{message || 'Loading runtime...'}</p>p>
                  </main>main>
                );
    if (!currentLesson)
          return (
                  <main className="p-8">
                          <p>Lesson not found.</p>p>
                  </main>main>
                );
    ...
    return (
          <main className="mx-auto max-w-4xl p-8 space-y-4">
                <h1 className="text-2xl font-semibold">
                        Learner Runtime: {currentLesson.title}
                </h1>h1>
                <p className="text-sm text-slate-500">
                        Lesson {lessonIndex + 1} of {runtime.lessons.length}
                </p>p>
          
                <button
                          className="rounded bg-slate-900 text-white px-3 py-1"
                          onClick={startLesson}
                        >
                        Start Lesson
                </button>button>
                ...
                <button
                          className="rounded bg-emerald-700 text-white px-3 py-1"
                          onClick={completeLesson}
                        >
                        Complete Lesson + Next
                </button>button>
          
                <pre className="rounded border bg-white p-3 text-xs overflow-auto max-h-48">
                  {message}
                </pre>pre>
          </main>main>
        );
