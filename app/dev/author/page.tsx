...
export default function AuthorDevPage() {
  ...
  return (
        <main className="mx-auto max-w-4xl p-8 space-y-4">
              <h1 className="text-2xl font-semibold">Author Flow Dev Page</h1>h1>
              ...
              <div className="flex gap-2 flex-wrap">
                      <button
                                  className="rounded bg-amber-700 text-white px-3 py-1"
                                  onClick={devSetup}
                                >
                                0) Dev Setup
                      </button>button>
                      <button
                                  className="rounded bg-slate-800 text-white px-3 py-1 disabled:opacity-40"
                                  onClick={seedData}
                                  disabled={!adminToken}
                                >
                      ...
                      <button
                                  className="rounded bg-blue-700 text-white px-3 py-1 disabled:opacity-40"
                                  onClick={submitReview}
                                  disabled={!seed?.courseVersionId}
                                >
                                2) Submit Review
                      </button>button>
                      <button
                                  className="rounded bg-indigo-700 text-white px-3 py-1 disabled:opacity-40"
                                  onClick={approve}
                                  disabled={!reviewId}
                                >
                                3) Approve
                      ...
                      <button
                                  className="rounded bg-emerald-700 text-white px-3 py-1 disabled:opacity-40"
                                  onClick={publish}
                                  disabled={!seed?.courseVersionId}
                                >
                                4) Publish
          ...
          {setup && (
                  <p className="text-sm text-green-700">
                            Logged in as: {setup.admin.email} (admin) | Token:{' '}
